...
mod tests {
    ...
    #[test]
    fn excludes_additional_default_artifact_directories() {
        let temp = TempDir::new().unwrap();
        let workspace_root = fs::canonicalize(temp.path()).unwrap();
        fs::create_dir_all(workspace_root.join("src")).unwrap();
    ...
    #[test]
    fn excludes_nested_default_artifact_directories() {
        let temp = TempDir::new().unwrap();
        let workspace_root = fs::canonicalize(temp.path()).unwrap();
        fs::create_dir_all(workspace_root.join("packages/a/src")).unwrap();
        fs::write(
            workspace_root.join("packages/a/src/lib.ts"),
            "export const lib = 1;\n",
        )
        .unwrap();
