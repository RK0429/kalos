...
**注意**: 本メモは設計判断の履歴記録である。全判断は v0.3.0（初回）、v0.4.0、v0.4.1 の各バッチで対象文書に適用済みである。各判断の現行仕様は対象文書（requirements.md / architecture.md / domain_model.md / ADR）の最新版を正本とすること。本メモ内の § 参照は初回適用前の文書構成に基づくため、セクション番号の軽微なずれが生じうる。
