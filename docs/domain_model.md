...
- `DiagnosticReport.summary_scope` は summary と exit code がどの母集団に対する集計かを表す。`SummaryScope.WholeProject`（JSON 値: `"whole_project"`）は non-diff モードの `--level all` において summary の母集団が解決済み `analysis_targets` 内の全階層の診断であることを表す。`DiagnosticReport.summary` は materialized value であり、`SummaryScope.ListedDiagnostics`（JSON 値: `"listed_diagnostics"`）では現在の `diagnostics` 一覧から Application Pipeline が再構成してから `DiagnosticReport` へ束ねる。diff mode では `summary_scope = ListedDiagnostics` を常に用い、affected diagnostics のみを母集団とする
