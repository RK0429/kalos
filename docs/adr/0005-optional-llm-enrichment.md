...
- **Outbound 通信**: `--llm` は LLM エンドポイントへのネットワークアクセスを暗示する（REQ-NF-009）。エンドポイント URL は環境変数 `KALOS_LLM_ENDPOINT_URL` で設定する。未設定時はプロバイダ固有のデフォルト URL へフォールバックする（REQ-NF-009 参照）。接続先エンドポイント URL は info レベルでログ出力する（ペイロードは出力しない）
