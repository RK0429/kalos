...
| 2026-03-19 | `BaselineFingerprint` 7 要素の定義、`SummaryScope` variant の明文化、保存不変条件・write-back 契約追加、コア評価順序の固定規則追加 | architecture.md v0.2.0–v0.2.8 |
| 2026-03-20 | `ScopeDiagnosticSnapshot` 保存単位、project scope 再計算規則、プラグインメトリクスのベースライン再利用ゲート追加 | architecture.md v0.2.12 |
