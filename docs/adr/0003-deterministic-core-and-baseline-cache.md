...
選択肢 B を採用する。本 ADR は「差分解析時に非変更スコープの結果をどう扱うか」を決定境界とし、ベースラインキャッシュからの再利用と影響範囲のみの再計算を採用する。決定論的コア評価パイプラインの出力一致性（`REQ-NF-003`）は本方式の下で維持する。

## 根拠

- `REQ-FUNC-024/034` を両立するには、非変更部分のベースライン再利用が最も自然
- `scores.overall` は常にメトリクス集約結果を表し、`SummaryScope` 列挙型の variant（`WholeProject` / `ListedDiagnostics`）は summary と exit code の母集団だけを規定する。non-diff モードの `--level all` 明示指定時は `SummaryScope.WholeProject`（JSON wire value: `"whole_project"`）を使い、デフォルトの `--level project`、`--level function|module|project`、diff mode では `SummaryScope.ListedDiagnostics`（JSON wire value: `"listed_diagnostics"`）を使う。差分モードでは summary / exit code だけを affected diagnostics に揃え、`scores.overall` は引き続き post-change メトリクス集約結果をそのまま反映する
